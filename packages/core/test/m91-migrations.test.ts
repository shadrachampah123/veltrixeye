...
test('M9.1 migrations upgrade an existing 0022 database and preserve email preference semantics', async () => {
  ...
  // Later milestones ship migrations after M9.1 (M9.2 added 0028, M10 Gate 9
  // added 0029), so assert the M9.1 set by name instead of by tail position.
  for (const file of [
    '0023_notification_preferences.sql',
    '0024_notification_routing.sql',
    '0025_webhook_tenant_integrity.sql',
    '0026_notification_delivery_fairness.sql',
    '0027_notification_fairness_ledger.sql',
  ]) {
    assert.ok(status.pending.includes(file), `${file} must still be pending for a 0022 database`);
  }
  const files = readdirSync(MIGRATIONS_DIR).filter((file) => file.startsWith('0023_') || file.startsWith('0024_') || file.startsWith('0025_') || file.startsWith('0026_') || file.startsWith('0027_') || file.startsWith('0028_'));
  ...
  // database (0013/0026 untouched; 0027/0028 additive with safe defaults).
  const fairness = await pool.query<{ e: string; w: string; p: string; last: string }>(
    'SELECT email_claims::text AS e, webhook_claims::text AS w, COALESCE(push_claims,0)::text AS p, last_channel AS last FROM notification_delivery_fairness WHERE singleton = true',
  );
  assert.deepEqual(fairness.rows[0], { e: '0', w: '0', p: '0', last: 'webhook' }, 'a live database upgraded in place starts from balanced durable fairness');
...
test('M9.1 migrations apply cleanly to a fresh database', async () => {
  ...
  // A fresh database applies every shipped migration (M9.1's five plus later
  // milestones), so assert the M9.1 files by name and the count by lower bound.
  for (const file of [
    '0023_notification_preferences.sql',
    '0024_notification_routing.sql',
    '0025_webhook_tenant_integrity.sql',
    ...
    assert.ok(result.applied.includes(file), `${file} must be applied on a fresh database`);
  }
