...
test('M9.1 migrations upgrade an existing 0022 database and preserve email preference semantics', async () => {
  ...
  // M9.2 adds 0028, so pending tail now includes 6 files; we assert the M9.1 set is still present
  const pendingTail = status.pending.slice(-6);
  assert.ok(pendingTail.includes('0023_notification_preferences.sql'));
  assert.ok(pendingTail.includes('0024_notification_routing.sql'));
  assert.ok(pendingTail.includes('0025_webhook_tenant_integrity.sql'));
  assert.ok(pendingTail.includes('0026_notification_delivery_fairness.sql'));
  assert.ok(pendingTail.includes('0027_notification_fairness_ledger.sql'));
