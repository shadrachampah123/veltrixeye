...
describe('outbox — claiming', () => {
  ...
  test('limit one fairness survives reconstruction with both queues continuously populated', async () => {
    ...
    for (let i = 0; i < 8; i++) {
      const reconstructed = new NotificationOutbox(pool, { maxAttempts: TEST_POLICY.maxAttempts });
      claimed.push(...await reconstructed.claimBatch(1, `restarted-worker-${i}`));
    }
  ...
  test('concurrent reconstructed workers share single-row fairness', async () => {
    const owner = await makeUser();
    for (let i = 0; i < 8; i++) await enqueueAlert(await makeAlert(owner.id), owner.id);
    for (let i = 0; i < 8; i++) await enqueueWebhook(await makeAlert(owner.id), owner.id);
    const batches = await Promise.all(Array.from({ length: 8 }, (_, i) =>
      new NotificationOutbox(pool, { maxAttempts: TEST_POLICY.maxAttempts }).claimBatch(1, `parallel-worker-${i}`),
    ));
  ...
  test('single available queue is claimed and fairness resumes when the other appears', async () => {
    ...
    const first = await new NotificationOutbox(pool, { maxAttempts: TEST_POLICY.maxAttempts }).claimBatch(1, 'available-only');
    assert.equal(first[0]?.channel, 'email');
