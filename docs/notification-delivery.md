...
## 11. M9.1 — preferences, routing, webhook, durable fairness

- `notification_preferences` (owner-scoped, email/webhook, enabled, endpoint_url, signing_secret) + `notification_user_settings` (quiet hours) + `strategy_notification_preferences` (muted + channels routing).
- Webhook outbox `notification_webhook_deliveries` separate table (0013 email-only), with tenant integrity composite FKs `(alert_id,user_id)` and `(alert_id,strategy_id)` via `0025`.
- Webhook provider with SSRF protection (HTTPS only, no URL creds, resolves all DNS, rejects private/reserved, pinned address, no redirect, bounded deadline), HMAC signing.
- Durable fairness: `notification_delivery_fairness` singleton, advisory lock `611_231_008` + `FOR UPDATE`, lifetime `email_claims, webhook_claims` in `0027`, transactional, immune to cleanup/cascade/retry/stale/restart, single and batch claims.

## 12. M9.2 — push channel, preferences UI, secret hardening, 3-way fairness

- Channels: `email | webhook | push`, max 3 in preferences and strategy routing, strict push subscription schema `{endpoint: https URL, keys: {p256dh: base64url, auth: base64url}}`, never exposed in DTOs.
- SecretManager: AES-256-GCM with `WEBHOOK_SECRET_ENCRYPTION_KEY` (32B base64), random IV 12B, authTag 16B, versioned `v<version>:<base64>`, key-version column, production fails closed if key missing/invalid, NoopSecretManager only for test/dev, redaction of keys, Render env vars alone do NOT constitute DB protection.
- Migration `0028_push_channel_and_secret_hardening.sql` additive: `notification_push_deliveries` table (channel push, recipient endpoint, signing_secret JSON keys encrypted, FKs, indexes, trigger), encrypted columns `signing_secret_encrypted` + `signing_secret_key_version` on preferences and webhook/push deliveries, `push_claims` on fairness, channel CHECKs extended safely via DO blocks inspecting `pg_constraint` (no assumed names), cardinality <=3, allowed ARRAY includes push, last_channel includes push, existing data preserved, plaintext secrets migration path encrypted without logs.
- Push provider: `createPushNotificationProvider` reuses `NotificationProvider`, uses `web-push` package, config VAPID_PUBLIC_KEY/PRIVATE_KEY/SUBJECT, PUSH_ENABLED, PUSH_PROVIDER_TIMEOUT_MS, private key never in describe/logs/errors/audit, endpoint HTTPS validation, outcome 200/201 delivered, 404/410 permanent, 429 retryable, 5xx retryable, timeout retryable, expired subscription safe.
- Three-way fairness: ledger `email_claims, webhook_claims, push_claims`, `last_channel` email|webhook|push, queue with fewer lifetime claims preferred, balanced continues round-robin from last_channel (canonical email→webhook→push), every committed claim increments counter transactionally under advisory lock, rollback rolls back, cleanup never decrements, retries/stale never increment, restart cannot reset, batch quotas fairly distributed (e.g. limit 5 → [2,2,1] in preference order) and remaining capacity filled when a queue empty (no stranded capacity), no worker-ID hash/parity.
- Preferences service: takes SecretManager, encrypts webhook secrets and push keys on write (stores encrypted, nulls plaintext), decrypts on deliveryTargets (prefers encrypted, fallback plaintext), owner-scoped, HTTPS-only, write-only API, GET never returns secrets, preserves quiet hours, strategy routing, email fallback.
- API: GET/PUT `/api/notifications/preferences` supports push, GET/PUT strategies supports push max 3, GET `/api/notifications/push/vapid-public-key` authenticated returns publicKey, POST `/api/notifications/push/subscriptions` authenticated owner-scoped strict validation encrypted at rest audit truncated endpoint, DELETE `/api/notifications/preferences/:channel` narrowly scoped. Preserves constant-time token, rate limiting, 404 masking, auth, tenant scoping.
- Web UI: Settings includes `NotificationPreferencesPanel` with email/webhook/push toggles, webhook URL + write-only secret, push permission state (unsupported/default/granted/denied), subscribe/unsubscribe via serviceWorker `pushManager.subscribe` with VAPID key, quiet hours, strategy routing, error/loading states, never displays secrets/keys, centralized BRAND, service worker `public/sw.js` handles push events (showNotification) and notificationclick (open `/alerts/:id`), pushsubscriptionchange resubscribe best-effort.
- Config/Render: `WEBHOOK_SECRET_ENCRYPTION_KEY`, `VAPID_PUBLIC_KEY/PRIVATE_KEY/SUBJECT`, `PUSH_ENABLED`, `PUSH_PROVIDER_TIMEOUT_MS` documented, no secrets in repo, production requires encryption key, VAPID private server-only, no deploy during task.
- Tests: contracts, migrations fresh and upgrade, secret manager, preferences encrypted, push provider, outbox 3-way fairness, API, web, regression all green, M9.1 fairness tests remain green, typecheck/build pass, M8.7 zero-diff, M9.1 0026/0027 zero-diff, production fail-closed confirmed.
